...
// Handle fetching departments for the frontend mapping
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // SECURITY OVERRIDE: Fetch the department directly from the database based on user_id
    // This prevents users from inspecting the element and changing their department
    $dept_name = 'General / Unassigned';
    $dept_query = $conn->query("SELECT d.dept_name, u.dept_id FROM user_dataset u LEFT JOIN departments d ON u.dept_id = d.dept_id WHERE u.user_id = '$user_id'");
    
    if ($dept_query && $dept_query->num_rows > 0) {
        $u_row = $dept_query->fetch_assoc();
        if (!empty($u_row['dept_name'])) {
            $dept_name = $u_row['dept_name'];
        } elseif (!empty($u_row['dept_id']) && $u_row['dept_id'] !== 'nan') {
            $dept_name = $u_row['dept_id']; // Fallback to ID if name is missing
        }
    }
    $department = $conn->real_escape_string($dept_name);
    ...
    if ($conn->query($sql) === TRUE) {
        echo json_encode(['status' => 'success', 'message' => 'Issue logged successfully for ' . $department . '.']);
    } else {
